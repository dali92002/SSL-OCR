...
class Transformer(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
        ...
        if custom_encoder is not None:
            ...
            self.to_patch, self.patch_to_emb = custom_encoder.to_patch_embedding[:2]
        else:
    ...
    def forward(self, src: Tensor, tgt: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        memory = self.enc_to_dec(encoded_tokens)
        memory = rearrange(memory, 'b t e -> t b e')

        src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = create_mask(memory[:,:,0], tgt[:,:,0])
